...
Plot `MT5.GetSymbol` output using [quantmod](https://CRAN.R-project.org/package=quantmod):
...
```{r openorder, eval=F, message=FALSE}
...
Looking for more examples? Good! Check **Articles** tab for even more (great!) examples. (UNDER CONSTRUCTION)
...
For _"Plug and play"_ version `.ex5` (`mt5R v0_1_0.ex5`)

1. Download [`mt5R v0_1_0.ex5`](https://github.com/Kinzel/mt5R/raw/main/MT5%20files/mt5R%20v0_1_0.ex5).
2. Move `mt5R v0_1_0.ex5` to `...\MQL5\experts\` folder. To check where MT5's folders, open MT5 and `Tools > Options > Storage`.
...
**Alternatively**, for MQL developers you can also download the `.mq5` code used to generate `mt5R v0_1_0.ex5`. Both are exactly the same code, it's the no compiled version. This is the choice that I recommend for who want to tailor their own solutions and functions.

* For non compiled version `.mq5` (`mt5R v0_1_0.mq5`) - The most suitable for developers.
  ...
  + If everything run smooth an `mt5R v0_1_0.ex5` (or the name you chose) will be generated
  
***
  
### Getting Started

Attach `mt5R v0_1_0.ex5` to any chart opened in your MT5, like any other Expert Advisor.
...
The most awesome part of MT5 is make reliable to create your owns solutions and functions. The cookbook is still under construction, but you can start checking functions `MT5.zExample` in mt5R package and `Z1` order in `mt5R v0_1_0.mq5`. Its pretty easy!
